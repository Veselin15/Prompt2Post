...
export default function DesignOptionsFields({
  ...
  return (
    <div className="space-y-3.5">
      {/* ── Essentials — everything a first post needs ─────────────────────── */}
      ...
      {/* ── Audience & goal — the biggest levers on what the copy says ──────── */}
      <SectionTitle>Audience &amp; goal</SectionTitle>
      ...
      <div>
        ...
        <input
          type="text"
          value={audience}
          onChange={(e) => onChange("audience", e.target.value.slice(0, 60))}
          placeholder="e.g. busy parents, indie founders, gym beginners…"
          maxLength={60}
          className="w-full bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-sm text-white placeholder-white/25 outline-none focus:border-brand-500/50 transition-colors"
        />
        <div className="flex gap-1.5 flex-wrap mt-1.5">
          {AUDIENCE_PRESETS.map((a) => (
            <button
              key={a}
              type="button"
              onClick={() => onChange("audience", audience.trim() === a ? "" : a)}
        ...
        <div className="flex gap-1.5 flex-wrap">
          {GOAL_OPTIONS.map((g) => (
            <Pill
              ...
              title={g ? GOALS[g].hint : "Let AI pick the best objective"}
            >
              {g ? GOALS[g].label : "Auto"}
            </Pill>
          ))}
        </div>
      </div>

      {/* ── Advanced — style, layout & brand controls (collapsed by default) ── */}
      <div className="pt-1">
        <button
          type="button"
        ...
        >
          <span className="inline-flex items-center gap-1.5 text-[10px] uppercase tracking-wider text-white/40 font-semibold group-hover:text-white/70 transition-colors">
            <Sliders className="w-3 h-3" />
            Advanced — style &amp; layout
          </span>
          <ChevronDown
            className={`w-3.5 h-3.5 text-white/40 group-hover:text-white/70 transition-all ${
                    ...
                    {TEMPLATE_OPTIONS.map((t) => {
                      ...
                      return (
                        <Pill
                          ...
                          title={t === "auto" ? "Let AI choose the layout" : OVERLAY_TEMPLATES[t].hint}
                          locked={isLocked}
                          lockedMsg={
                            t !== "auto"
                    ...
                    <div className="flex gap-1.5 flex-wrap">
                      {EMOJI_OPTIONS.map((e) => (
                        <Pill
                          ...
                          title={EMOJI_LEVELS[e].hint}
                        >
                          {EMOJI_LEVELS[e].label}
                        </Pill>
                      ))}
                    </div>
                  </div>
